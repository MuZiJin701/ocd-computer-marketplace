...
- The shared Palette remains the highest color-generation Seam. Add a `prediction_foreground` role because the confirmed requirement is stronger than ordinary muted text: it must be neutral, readable and perceptually distinct from `foreground`. No machine-specific colors are added.
- Light and Dark each generate their own `prediction_foreground` from the Plan Palette. The role must meet `contrast_ratio(prediction_foreground, surface) >= 4.5` and OKLab Euclidean distance `ΔE_OK >= 0.08` from `foreground`. The threshold is a deterministic acceptance rule, not a display-specific color constant.
...
- Palette and Terminal artifact tests verify `prediction_foreground` for Light and Dark, the `ΔE_OK >= 0.08` and `4.5:1` constraints, and that PSReadLine prediction colors are not confused with Terminal `foreground` or ANSI semantic colors.
