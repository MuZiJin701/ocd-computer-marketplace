...
3. Add a dedicated `prediction_foreground` Palette role. Generate it from Seed-derived complementary/triadic hue candidates using the existing OKLCH/OKLab implementation. Keep a shared hue family across Light and Dark, independently adapt tone and chroma to each Terminal surface, avoid nearby semantic text hues, require `ΔE_OK >= 0.20` and contrast `>= 4.5:1`, and target OKLCH chroma around `0.10` with a bounded relaxation ceiling. Map inline and unselected list prediction text to this role; map selected prediction to the existing selection foreground/background pair. Do not change prediction sources or key bindings.
...
- Prediction colors no longer depend on `muted_foreground`, whose contrast can be valid while its visual difference from `foreground` is too small. The Seed-derived hue search can use controlled chroma when strict neutrality cannot meet the visible-distance target.
...
- Requiring strict low-chroma neutrality or a fixed prediction hue, because the real Light-mode screenshot showed that this can pass contrast while remaining visually ambiguous.
