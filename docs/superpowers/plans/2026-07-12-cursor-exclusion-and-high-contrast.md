...
**Architecture:** Keep the existing Cursor adapter isolated but remove it from the production target registry and default target discovery. Replace sparse foreground selection with dense same-hue candidate search, add contrast-safe text variants for accent and semantic colors, then write `contrast=100` into both verified Codex chrome theme tables while preserving `appearanceTheme`.
...
- [x] **Step 1: Add a failing test for Cursor exclusion.** Assert the default adapter target names do not include `cursor`, and explicit `cursor` returns `skipped` without creating a settings or extension file.
- [x] **Step 2: Add failing contrast assertions.** For `#10B981`, require primary and emphasis text against `surface` to reach `5.5:1`, require `background_foreground/background >= 7`, and require the primary foreground to remain non-achromatic.
- [x] **Step 3: Add a failing Codex contrast assertion.** Apply a plan to a valid TOML fixture with `contrast=20`, then assert both light and dark theme tables contain `contrast=100` while `appearanceTheme` remains unchanged.
- [x] **Step 4: Run only the new tests and confirm RED.**
...
- [x] **Step 1: Remove `cursor` from the production registry and default detection list.** Leave `vscode_family.py` Cursor classes untouched for future experiments.
- [x] **Step 2: Return `skipped` for explicit `cursor` before adapter construction or filesystem access.** Use the existing structured `AdapterResult` path.
- [x] **Step 3: Run the Task 1 target tests and confirm GREEN.**
...
- [x] **Step 1: Expand same-hue candidates over lightness increments of at most `0.02` and saturation variants derived from the source color.** Exclude only exact `#000000` and `#FFFFFF` from normal candidates.
- [x] **Step 2: Add a candidate score that first satisfies the requested contrast threshold, then minimizes distance from readable lightness and maximizes retained saturation.** If no candidate satisfies the threshold, return the candidate with maximum contrast.
- [x] **Step 3: Generate `foreground` against `surface` at `5.5:1`; generate `muted_foreground` at `4.5:1`; generate `accent_text`, `error_text`, `warning_text`, and `success_text` against `surface`; generate accent and selection foregrounds at `5.5:1`.
- [x] **Step 4: Update validation pairs to match the generated semantic contracts and run all palette tests.**
...
- [x] **Step 1: Extend the verified replacement map to write `contrast=100` only inside the two appearance theme tables.** Keep all unrelated TOML text unchanged.
- [x] **Step 2: Extend `_matches_plan()` to require `contrast == 100` in both theme tables without requiring a particular `appearanceTheme`.
- [x] **Step 3: Run Codex adapter tests and confirm GREEN.**
...
- [x] **Step 1: Rewrite root README around installation and commands; remove implementation explanations and Cursor claims.**
- [x] **Step 2: Update support matrices, project rules, architecture, test guidance, Skill, and current plan references to omit Cursor from current support and describe the new contrast thresholds.**
- [x] **Step 3: Scan the distributable Skill for developer-machine paths and temporary paths.**
...
- [x] **Step 2: Run CLI help and a read-only palette smoke check.**
- [x] **Step 3: Run `git diff --check` and the package path scan.**
