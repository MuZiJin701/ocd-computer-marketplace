...
Palette tests also verify that the Seed is preserved exactly as `surface`, primary and semantic text variants are readable on `surface`, `background_foreground` is readable on the deep background, the Windows wallpaper is a solid Seed-colored PNG, Windows accents come from `accent`, and Apply preserves the user's mode and automatic colorization registry values. The contrast checks use 7:1 for deep-background text and 5.5:1 for primary/emphasis surface text; when a Seed's theoretical maximum is lower, the algorithm selects the maximum attainable contrast.
