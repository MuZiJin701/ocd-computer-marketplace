...
The runtime fixture suite covers cross-process-style adapter instances, transaction journaling, failed compensation, safe path components, atomic plan/transaction writes, environment path overrides, and Cursor launcher argument discovery. It does not prove real Windows registry, desktop wallpaper, editor CLI, or Chrome activation compatibility.
