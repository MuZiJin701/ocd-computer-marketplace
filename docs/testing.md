...
Palette tests also verify that the Seed is preserved exactly as `surface`, `foreground` is readable on `surface`, `background_foreground` is readable on the deep background, the Windows wallpaper is a solid Seed-colored PNG, Windows accents come from `accent`, and Apply preserves the user's mode and automatic colorization registry values. The contrast checks use 7:1 for deep-background text and 4.5:1 for actual surface/UI foreground pairs; a fixed 7:1 requirement against every arbitrary Seed would be mathematically impossible for some colors.
