# One-Tone Windows 主题核心

One-Tone 将同一个 Seed Color 生成的 Palette 应用于 Windows 11、Windows Terminal、VS Code Family、Codex 和 Chrome，并通过 Plan、Transaction 和回滚保持可验证。
...
Windows Adapter 使用 Palette 生成确定性的深色渐变 PNG，在 Apply 时设置桌面壁纸；事务会保存原壁纸路径和原文件，Rollback 恢复它们。事务目录位于 `.one-tone/transactions/<transaction_id>/`，只使用该事务自己的备份。
