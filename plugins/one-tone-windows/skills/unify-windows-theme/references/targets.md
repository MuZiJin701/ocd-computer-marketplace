...
| `cursor` | Cursor 通用工作台、主题扩展和 `workbench.colorCustomizations` fallback | Cursor 专属界面可能不受标准主题字段控制；VSIX 无法注册时使用设置 fallback，需重启后 Verify，Verify 会重新发现扩展目录 |
...
目标结果使用 `ok`、`partial`、`failed` 或 `skipped`，至少包含 `target`、`status`、`changed`、`verified` 和 `message`。Palette 的 `background_foreground` 对深层背景目标为 `>= 7:1`，`foreground` 对实际 `surface` 目标为 `>= 4.5:1`；Seed 本身不因对比度计算被暗化。
