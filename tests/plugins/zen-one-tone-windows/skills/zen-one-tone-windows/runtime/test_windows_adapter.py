...
def test_wallpaper_generation_is_png_and_deterministic(tmp_path):
    ...
    first = generate_wallpaper(plan.palette_for(plan.mode), tmp_path / "first.png", width=32, height=16)
    second = generate_wallpaper(plan.palette_for(plan.mode), tmp_path / "second.png", width=32, height=16)
...
def test_windows_apply_sets_green_accent_and_taskbar_prevalence(tmp_path):
    ...
    assert registry.values["AccentColorMenu"] == windows_color_value(plan.palette_for(plan.mode)["accent"])
    assert registry.values["ColorizationColor"] == windows_colorization_value(plan.palette_for(plan.mode)["accent"])
    assert adapter.verify(plan).verified is True


def test_windows_light_mode_attempts_taskbar_setting_and_reports_native_visual_gate(tmp_path):
    registry = InMemoryRegistryBackend({
        "CurrentBuild": "26200",
        "AppsUseLightTheme": 0,
    ...
    assert result.metadata["field_capabilities"]["StartTaskbarColorPrevalence"] == "partial"
    assert adapter.verify(plan).metadata["field_capabilities"]["StartTaskbarColorPrevalence"] == "partial"
