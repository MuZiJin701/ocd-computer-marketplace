...
def test_codex_v1_fixture_completes_config_lifecycle_and_preserves_fields(tmp_path):
    path = write_codex_v1_fixture(tmp_path / "config.toml")
    ...
    assert restart.verified is False
    assert restart.requires_user_action is True
